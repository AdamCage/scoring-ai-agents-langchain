...
def build_query(app: Application, extra: str = "") -> str:
    parts = [
...
def retrieve_policy(
    ...
    docs, vectors, bm25, tokens = _index()
    query = build_query(app, extra_query)
    allowed = _filter_docs(docs, app.segment)
    debug = RetrievalDebug(query=query, filters={"segment": app.segment})
    ...
    for idx, rerank_score in reranked[:top_k]:
        results.append(
            RetrievedDocument(
                ...
                score=round(float(fused.get(idx, 0.0) + rerank_score), 4),
                ...
                rrf_score=round(float(fused.get(idx, 0.0)), 4),
...
def knowledge_stats() -> dict:
    docs = load_documents()
    return {"documents": len(docs), "rag_version": RAG_VERSION, "ready": len(docs) > 0}
